...
- **初期ロータ位置 (Initial position / Grundstellung)**: 利用者が鍵として設定・共有する固定値
- **現在ロータ位置 (Current position)**: 文字変換に伴って進行する実行時状態
...
この値は「鍵として共有する設定値」であり、`Encrypt / Decrypt` 実行では書き換えません。

#### 現在ロータ位置（表示）

- 文字変換に応じて進む実行時の位置を表示する
- 初回実行前は初期ロータ位置と同じ値
- `Reset positions` で現在位置を初期ロータ位置に戻す
